...
async function run() {
  ...
  if (relRes.status === 404) {
    ...
    const releaseNotes = `### الإصدار المستقر v2.3.48 - معمل تعتيق العطور (عطور المعتق) وإدارة أصول التشغيل WIP 🌿🧪
 
- **معمل تعتيق العطور (Perfume Maceration & Aging Lab)**: وحدة صناعية ومحاسبية متكاملة لإدارة فترات تعتيق ونضج العطور المركبة (30-90 يوماً).
- **محرك التركيبة وحساب التكلفة التفاعلي اللحظي**:
  - حساب التكلفة التناسبية للزيوت الخام، كحول الإيثانول الطبي، المثبتات والمعدلات (DPG، ISO E Super)، والأوعية الزجاجية.
  - حساب فوري لنسبة تركيز الزيت وتصنيف العطر (Extrait de Parfum، Eau de Parfum، إلخ).
- **الحجز والخصم الذري للمخزون (WIP Inventory Commit)**:
  - خصم كميات الزيت والكحول والوعاء فورياً من أرصدة المخزون وحفظها كأصل قيد التشغيل (WIP) معزول تماماً عن نقاط البيع (POS).
- **تتبع الجدول الزمني للتعتيق وشريط التقدم التفاعلي**:
  - احتساب تاريخ الجاهزية تلقائياً وعرض شريط تقدم حي بالأيام والنسبة المئوية.
  - تنبيهات آلية للمدير في لوحة المعلومات وقسم التعتيق عند انتهاء مدة النضج.
- **مسارات التخريج والاعتماد المرنة**:
  - احتساب فاقد التبخر وإعادة اشتقاق التكلفة الحقيقية للملي الواحد.
  - تخريج كعطر سائب بالمل، أو تعبئة في قوارير فاخرة مع خصم الزجاجات الفارغة والعلب الكرتونية وإضافتها فورياً للمخزون.
- **تحديثات الاستقرار الشاملة**: اجتياز 38 حزمة اختبار و 268 فحصاً آلياً بنسبة 100%.`;
